...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            box = label.box
            boxA = tools.compute_box_corners(box.center_x, box.center_y, box.width, box.length, box.heading)
            ## step 2 : loop over all detected objects
            for det in detections:
                ## step 3 : extract the four corners of the current detection
                _, x, y, z, h, w, l, yaw = det
                boxB= tools.compute_box_corners(x, y, w, l, yaw)
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                dist_x = box.center_x - x
                dist_y = box.center_y - y
                dist_z = box.center_z - z
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                # The code below comes from https://pyimagesearch.com/2016/11/07/intersection-over-union-iou-for-object-detection/
                xA = max(boxA[0][0], boxB[0][0])
                yA = max(boxA[1][1], boxB[1][1])
                xB = min(boxA[2][0], boxB[2][0])
                yB = min(boxA[3][1], boxB[3][1])
                interArea = max(0, xB - xA + 1) * max(0, yB - yA + 1)
                boxAArea = (boxA[2][0] - boxA[0][0] + 1) * (boxA[3][1] - boxA[1][1] + 1)
                boxBArea = (boxB[2][0] - boxB[0][0] + 1) * (boxB[3][1] - boxB[1][1] + 1)
